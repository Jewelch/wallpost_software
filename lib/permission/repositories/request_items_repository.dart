...
class RequestItemsRepository {
  ...
  Future saveRequestItemsForEmployee(String companyId, List<RequestItem> requestItems) async {
  ...
  Future<List<RequestItem>> getRequestItemsOfCompany(String companyId) async {
  ...
  Future<List<RequestItem>> _readRequestItems(String companyId) async {
    var itemsMap = await _sharedPrefs.getMap(_key);
    List mappedItems = itemsMap?[companyId] ?? [];
    var items = mappedItems.map<RequestItem>((item) => initializeRequestFromString(item)!).toList();
    return items;
  }
