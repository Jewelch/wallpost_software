...
class ItemSalesProvider {
  ...
  Future<ItemSalesReport> getItemSales(DateRange dateRange) async {
    var companyId = _selectedCompanyProvider.getSelectedCompanyForCurrentUser().id;
    var url = ItemSalesUrls.getSalesItemUrl(companyId, dateRange);
  ...
  Future<ItemSalesReport> _processResponse(APIResponse apiResponse) async {
    //returning empty list if the response is from another session
    if (apiResponse.apiRequest.requestId != _sessionId) return Completer<ItemSalesReport>().future;
    if (apiResponse.data == null) throw InvalidResponseException();
    if (apiResponse.data is! Map<String, dynamic>) throw WrongResponseFormatException();

    var responseList = apiResponse.data as Map<String, dynamic>;
    try {
      return ItemSalesReport.fromJson(responseList);
    } catch (e) {
