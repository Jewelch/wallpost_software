...
class FinanceFilters extends StatefulWidget {
  ...
  FinanceFilters(
    ...
    this._initialMonth =
        _isMonthValidForYear(initialMonth, initialYear) ? initialMonth - 1 : -1;
    this._initialYear =
        _years.contains(initialYear) ? _years.indexOf(initialYear) : 0;
  }
  ...
  @override
  State<FinanceFilters> createState() =>
      _FinanceFiltersState(_initialMonth, _initialYear);
}
...
class _FinanceFiltersState extends State<FinanceFilters> {
                ...
                TextButton(
                  onPressed: () => [
                    ...
                    widget.presenter.setFilter(
                        ...
                        year: AppYears().years()[_selectedYear])
                  ],
                  child: Text(
                    "Apply"
                    "",
  ...
  Widget getSingleYearItem(int index) {
    return CustomFilterChip(
        backgroundColor: (index == _selectedYear)
  ...
  Widget getSingleMonthItem(int index) {
    return CustomFilterChip(
        backgroundColor: (index == _selectedMonth)
            ? AppColors.defaultColor
            : Colors.transparent,
        borderColor: Colors.transparent,
        title: Text(
  ...
  List<String> _getMonthNamesForSelectedYear() {
    var years =
        AppYears().currentAndPastShortenedMonthsOfYear(widget._years.last);
    return years;
  }
}
