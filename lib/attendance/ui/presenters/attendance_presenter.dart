...
class AttendancePresenter {
  ...
  AttendancePresenter.initWith(this._view, this._attendanceDetailsProvider, this._locationProvider,
      this._punchInFromAppPermissionProvider, this._punchInNowPermissionProvider, this._attendanceLocationValidator);
  ...
  Future<void> _loadPunchInDetails() async {
    try {
      ...
      _view.hideLoader();
      _view.showDisableButton();
      _view.showFailedToGetPunchInPermission("Loading punch in permission failed", e.userReadableMessage);
    }
  }
  ...
  Future<void> doPunchOut() async {
    var isLocationValid = await _attendanceLocationValidator.validateLocation(_attendanceLocation, isForPunchIn: true);
