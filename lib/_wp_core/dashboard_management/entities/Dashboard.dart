...
class Dashboard extends JSONInitializable implements JSONConvertible {
  late String _status;
  late List<CompaniesGroup> _data;

  Dashboard.fromJson(Map<String, dynamic> jsonMap) : super.fromJson(jsonMap) {
    var sift = Sift();
    try {
      _status = '${sift.readStringFromMap(jsonMap, 'status')}';
      sift.readMapListFromMap(jsonMap, 'data').forEach((v) {
        _data.add(CompaniesGroup.fromJson(v));
      });
  ...
  List<CompaniesGroup> get companies => _data;
