...
void main() {
  ...
  test('sets punch time to 00:00 at initializations if the punch times are null ', () async {
    //given
    when(() => attendanceListItem.punchInTime).thenReturn(null);
    when(() => attendanceListItem.punchOutTime).thenReturn(null);

    expect(presenter.getPunchInTime(),  TimeOfDay(hour: 00, minute: 00));
    expect(presenter.getPunchOutTime(), TimeOfDay(hour: 00, minute: 00));

    _verifyNoMoreInteractionsOnAllMocks();
  });

  test('sets punch in and out time to the actual punch in and punch out time at the time of initialization', () async {
    //given
    when(() => attendanceListItem.punchInTime).thenReturn(DateTime(2017, 9, 7, 17, 30));
    when(() => attendanceListItem.punchOutTime).thenReturn(DateTime(2017, 9, 7, 8, 45));


    expect(presenter.getPunchInTime(),  TimeOfDay(hour: 17, minute: 30));
    expect(presenter.getPunchOutTime(), TimeOfDay(hour: 8, minute: 45));
