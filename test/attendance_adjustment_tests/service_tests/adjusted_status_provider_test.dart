...
void main() {
  ...
  var adjustedAttendanceStatusProvider =
      AdjustedAttendanceStatusProvider.initWith(
  ...
  setUpAll(() {
    ...
    when(() => mockAttendanceAdjustmentForm.adjustedPunchInTime)
        .thenReturn('09:00');
    when(() => mockAttendanceAdjustmentForm.adjustedPunchOutTime)
        .thenReturn('06:00');
  ...
  test('api request is built correctly', () async {
    ...
    var _ = await adjustedAttendanceStatusProvider
        .getAdjustedStatus(mockAttendanceAdjustmentForm);

    expect(
        mockNetworkAdapter.apiRequest.url,
        AttendanceAdjustmentUrls.getAdjustedStatusUrl(
            'someCompanyId', 'v1EmpId', 'date', '09:00', '06:00'));
