...
void main() {
  ...
  var salesBreakDownsProvider = SalesBreakDownsProvider.initWith(
    mockNetworkAdapter,
  ...
  test('SalesBreakDown Provider request is built and executed correctly',
      () async {
    Map<String, dynamic> requestParams = {};
    mockNetworkAdapter.succeed(successfulResponse);

    await salesBreakDownsProvider.getSalesBreakDowns(
        salesBreakDownWiseOption, dateFilter);
  ...
  test(
      'SalesBreakDown Provider throws <NetworkFailureException> when network adapter fails',
      () async {
  ...
  test(
      'SalesBreakDown Provider throws <InvalidResponseException> when response is null',
      () async {
  ...
  test(
      'SalesBreakDown Provider throws <WrongResponseFormatException> when response is of the wrong format',
      () async {
  ...
  test(
      'SalesBreakDown Provider throws <InvalidResponseException> when entity mapping fails',
      () async {
  ...
  test(
      'SalesBreakDown Provider response is ignored if it is from another session',
      () async {
    ...
    salesBreakDownsProvider
        .getSalesBreakDowns(salesBreakDownWiseOption, dateFilter)
        .then((_) {
  ...
  test('test loading flag is set to true when the service is executed',
    ...
    salesBreakDownsProvider.getSalesBreakDowns(
        salesBreakDownWiseOption, dateFilter);
