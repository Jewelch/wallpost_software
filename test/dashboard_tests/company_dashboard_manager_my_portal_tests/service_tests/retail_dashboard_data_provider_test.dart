...
import 'package:wallpost/dashboard/company_dashboard_manager_my_portal/constants/manager_my_portal_dashboard_urls.dart';
import 'package:wallpost/dashboard/company_dashboard_manager_my_portal/services/retail_dashboard_data_provider.dart';
...
void main() {
  Map<String, dynamic> successfulResponse = Mocks.retailDashboardDataResponse;
  var mockCompanyProvider = MockCompanyProvider();
  var mockNetworkAdapter = MockNetworkAdapter();
  var dataProvider = RetailDashboardDataProvider.initWith(mockCompanyProvider, mockNetworkAdapter);
  ...
  test('api request is built and executed correctly', () async {
    ...
    expect(mockNetworkAdapter.apiRequest.url, ManagerMyPortalDashboardUrls.retailDataUrl('someCompanyId', 1, 2022));
  ...
  test('month is set to null if it is 0', () async {
    ...
    var _ = await dataProvider.get(month: 0, year: 2022);

    expect(mockNetworkAdapter.apiRequest.url, ManagerMyPortalDashboardUrls.retailDataUrl('someCompanyId', null, 2022));
  });
  ...
  test('success', () async {
    ...
    try {
      var attendanceDetails = await dataProvider.get(month: 1, year: 2022);
