...
import 'package:wallpost/dashboard/company_dashboard_owner_my_portal/ui/presenters/crm_performance_presenter.dart';
...
void main() {
  ...
  setUp(() {
    presenter = CRMPerformancePresenter.initWith(view, filters, dataProvider);
  });
  ...
  test("get actual revenue", () async {
    ...
    expect(presenter.getActualRevenue().label, "Actual Revenue");
    expect(presenter.getActualRevenue().value, "-120");
    expect(presenter.getActualRevenue().textColor, AppColors.red);
    ...
    expect(presenter.getActualRevenue().textColor, AppColors.green);
  });
