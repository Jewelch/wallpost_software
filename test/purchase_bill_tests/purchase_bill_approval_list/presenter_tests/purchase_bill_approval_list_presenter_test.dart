...
void main(){
  ...
  test("successfully performing action on one of three items updates the list", () async {
    ...
    expect(presenter.getItemTypeAtIndex(0), PurchaseBillApprovalListItemViewType.ListItem);
    expect(presenter.getItemTypeAtIndex(1), PurchaseBillApprovalListItemViewType.ListItem);
    expect(presenter.getItemTypeAtIndex(2), PurchaseBillApprovalListItemViewType.Loader);
    verifyInOrder([
          () => view.updateList(),
          () => listProvider.isLoading,
  ...
  test("does not reload data when processing approval or rejection with null or false", () async {
    presenter.onDidProcessApprovalOrRejection(null, ["someBillId"]);
    _verifyNoMoreInteractions();

    presenter.onDidProcessApprovalOrRejection(false, ["someBillId"]);
    _verifyNoMoreInteractions();
  });
